...
export class SearchboxComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.marketService.sym$.subscribe(data => {
      this.sym = data;
      if (this.sym != undefined) {
        this.subscription = timer(0, 30000).pipe(
          switchMap(() => this.marketService.getTimeSeries(this.quoteControl.value.apiname, this.sym.symbol, "full", this.numControl.value.value))
        ).subscribe(result => {
          console.log(result);
          //search for the dataobject with the same symbol name, if it is the same, update it if not push it;
          var fl = false;
          for (var i = 0; i < this.dataArray.length; i++) {
            if (this.dataArray[i]['Meta Data']['2. Symbol'] == result['Meta Data']['2. Symbol']) {
              this.dataArray[i] = result;
              fl = true;
            }
          }
          if (fl == true) { } else { this.dataArray.push(result); }
          this.marketService.updateData$.next(this.dataArray);
        }) //push to array, and push array to the service;
      }
    });
    this.marketService.updateData$.subscribe(data => {
      console.log(data);
    });
  }

  public assign(value: string) {
    this.marketService.getSym(value);
  }
